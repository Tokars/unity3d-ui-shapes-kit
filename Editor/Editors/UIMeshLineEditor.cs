...
namespace UIShapeKit.Editor.Editors
{
    ...
    public class UIMeshLineEditor : UnityEditor.Editor
    {
        UIMeshLine m_UIMeshLine;

        UIMeshLine owner => m_UIMeshLine ?? (m_UIMeshLine = target as UIMeshLine);
        ...
        void HandlesForPoints()
        {
            ...
            for (int n = 0; n < owner.GetPointCount(); n++)
            {
                ...
                {
                    serializedObject.FindProperty("m_points").GetArrayElementAtIndex(n).FindPropertyRelative("point")
                        .vector2Value = ownerTrans.InverseTransformPoint(point);
                ...
                if (owner.GetPointInfo(n).isNextCurve)
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        serializedObject.FindProperty("m_points").GetArrayElementAtIndex(n)
                                .FindPropertyRelative("nextCurveOffset").vector2Value
                            = cp0 - point;
                        serializedObject.ApplyModifiedProperties();
                    }
                }
                ...
                if (owner.GetPointInfo(n).isPrvCurve)
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        serializedObject.FindProperty("m_points").GetArrayElementAtIndex(n)
                                .FindPropertyRelative("prvCurveOffset").vector2Value
                            = cp1 - point;
                        serializedObject.ApplyModifiedProperties();
                    }
                }
